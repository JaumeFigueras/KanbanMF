const en = {
  signIn: {
    title: 'Sign in to KanbanMF',
    email: 'Email',
    password: 'Password',
    submit: 'Sign in',
    noAccount: "Don't have an account?",
    signUpLink: 'Sign up',
    errorCredentials: 'Incorrect email or password',
    errorNotVerified: 'Please verify your email address before signing in.',
  },
  signUp: {
    title: 'Create your account in KanbanMF',
    displayName: 'Display name',
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm password',
    passwordMismatch: 'Passwords do not match',
    submit: 'Create account',
    hasAccount: 'Already have an account?',
    signInLink: 'Sign in',
    emailTaken: 'This email address is already registered.',
    errorGeneric: 'Something went wrong. Please try again.',
    checkEmailTitle: 'Check your email',
    checkEmailBody: 'We sent a verification link to your email address. Please click it to activate your account.',
  },
  verifyEmail: {
    verifying: 'Verifying your email…',
    successTitle: 'Email verified!',
    successBody: 'Your account is now active. You can sign in.',
    signInNow: 'Sign in',
    errorTitle: 'Verification failed',
    errorExpired: 'This verification link has expired. Please sign up again to get a new one.',
    errorInvalid: 'This verification link is invalid or has already been used.',
    tryAgain: 'Back to sign up',
  },
  boards: {
    createNewBoard: 'Create a new Board',
    boardName: 'Board name',
    boardNameRequired: 'Board name is required.',
    starred: 'Starred',
    starredBoards: 'Starred',
    myBoards: 'My Boards',
    sharedWithMe: 'Shared with me',
    noBoardsYet: 'No boards here yet.',
    signOut: 'Sign out',
    userProfile: 'User profile',
    changeDisplayName: 'Change display name and initials',
    changePassword: 'Change password',
    uploadAvatar: 'Upload avatar',
    changeToDark: 'Change to Dark Theme',
    changeToLight: 'Change to Light Theme',
    languageLocalization: 'Language and Localization',
    displayLanguage: 'Display language',
    dateTimeLocale: 'Dates & times format',
    dateTimeExample: 'Example:',
    timezone: 'Timezone',
    dateDisplayFormat: 'Date display on cards',
    dateFormatNumeric: 'Numeric (e.g. 27/06/2026)',
    dateFormatTextual: 'Textual (e.g. 27 June 2026)',
    initials: 'Initials',
    initialsHint: 'Up to 3 letters. Leave blank to use display name initials.',
    displayNameRequired: 'Display name cannot be blank.',
    initialsInvalid: 'Initials must be 1–3 letters or digits.',
    oldPassword: 'Current password',
    newPassword: 'New password',
    oldPasswordRequired: 'Current password is required.',
    newPasswordRequired: 'New password is required.',
    confirmPasswordRequired: 'Please confirm your new password.',
    wrongOldPassword: 'Current password is incorrect.',
    changeRemoveAvatar: 'Change / Remove avatar',
    removeAvatar: 'Remove avatar',
    chooseFile: 'Choose image…',
    avatarSizeLimit: 'Maximum file size: 100 KB. Accepted formats: JPEG, PNG, WebP, GIF.',
    avatarTypeError: 'Only image files are allowed (JPEG, PNG, WebP, GIF).',
    avatarSizeError: 'File is too large. Maximum size is 100 KB.',
    noFileSelected: 'Please select an image file.',
    changeBoardName: 'Change Name',
    changeBoardColor: 'Change Color',
    resetColorToDefault: 'Reset to default',
    archiveBoard: 'Archive Board',
    star: 'Star',
    unstar: 'Unstar',
    reorderHint: 'Reorder',
    shareBoard: 'Share',
    shareBoardTitle: 'Share board',
    sharedWith: 'Shared with',
    noSharedUsers: 'Not shared with anyone yet.',
    searchUserPlaceholder: 'Search by name or email…',
    noUsersFound: 'No users found.',
    removeShareConfirmTitle: 'Remove from board?',
    removeShareConfirmMessage: 'Remove {{name}} from this board? They will lose access.',
    remove: 'Remove',
    archiveBoardConfirmMessage: 'Are you sure you want to archive "{{name}}"? It will be hidden from your boards list.',
    archive: 'Archive',
    showArchivedBoards: 'Show Archived Boards',
    archivedBoards: 'Archived Boards',
    restoreBoard: 'Restore Board',
    deleteBoard: 'Delete Board',
    deleteBoardConfirmMessage: 'Are you sure you want to permanently delete "{{name}}"? This cannot be undone.',
    delete: 'Delete',
    emailNotification: 'E-Mail notification',
    emailNotificationTitle: 'My e-mail notifications',
    emailNotificationHint: 'These settings are personal to you — turning them on only affects e-mails sent to you, never to anyone else on this board.',
    enableEmailNotifications: 'E-mail me about my due dates on this board',
    notifyHour: 'Notification hour',
    notifyOffsetDays: 'Notify these days before/after the due date',
    noOffsetDays: 'No notification days configured yet.',
    offsetDaysPlaceholder: 'e.g. -3, -1, 0, 1',
    overdueRepeatPrefix: 'Send e-mails daily past',
    overdueRepeatSuffix: 'days the due date',
  },
  board: {
    addList: 'Add List',
    boardMenu: 'Board menu',
    archivedTitle: 'Board Archived',
    archivedMessage: 'This board has been archived and is no longer accessible.',
    forbiddenTitle: 'Forbidden',
    forbiddenMessage: 'You don\'t have access to this board.',
    backToBoards: 'Back to Boards',
    createList: 'Create a new List',
    listName: 'List name',
    listNameRequired: 'List name is required.',
    archiveAllCards: 'Archive All Cards',
    renameList: 'Rename List',
    changeListColor: 'Change Color',
    archiveList: 'Archive List',
    listMenu: 'List menu',
    addCard: 'Add card',
    moveList: 'Move list',
    manageLabels: 'Labels',
    noLabels: 'No labels yet.',
    createNewLabel: 'Create New Label',
    labelName: 'Label name',
    labelNameRequired: 'Label name is required.',
    editLabel: 'Edit label',
    deleteLabel: 'Delete label',
    moveLabel: 'Move label',
    createCard: 'Create Card',
    editCard: 'Edit Card',
    changeCardColor: 'Change Color',
    cardName: 'Card name',
    cardNameRequired: 'Card name is required.',
    cardDescription: 'Description',
    moveCard: 'Move card',
    cardMenu: 'Card menu',
    copyCard: 'Copy Card',
    copyCardNamePlaceholder: 'Name for the copy',
    targetBoard: 'Board',
    targetList: 'List',
    noListsOnBoard: 'This board has no lists yet.',
    targetCard: 'Card',
    archiveCard: 'Archive card',
    dates: 'Dates',
    startDate: 'Start Date',
    dueDate: 'Due date',
    endDate: 'End date',
    dateOrderError: 'Start date must be before the end date.',
    invalidDateTime: 'Invalid date or time.',
    dueLabel: 'Due',
    endLabel: 'End',
    clearDate: 'Remove date',
    labels: 'Labels',
    selectLabels: 'Select Labels',
    manageCardLabels: 'Add labels',
    people: 'People',
    creator: 'Creator',
    members: 'Members',
    assignees: 'Assignees',
    manageMembers: 'Add members',
    manageAssignees: 'Add assignees',
    selectMembers: 'Select Members',
    selectAssignees: 'Select Assignees',
    noUsersAvailable: 'No users available yet.',
    checklists: 'Checklists',
    addChecklist: 'Add checklist',
    noChecklists: 'No checklists yet.',
    createChecklist: 'Create Checklist',
    editChecklist: 'Edit Checklist',
    deleteChecklist: 'Delete Checklist',
    checklistMenu: 'Checklist menu',
    copyChecklist: 'Copy checklist to another card',
    copyChecklistTitle: 'Copy Checklist',
    moveChecklist: 'Move checklist',
    checklistName: 'Checklist name',
    checklistNameRequired: 'Checklist name is required.',
    checklistItems: 'Items',
    noChecklistItems: 'No items yet.',
    addChecklistItem: 'Add an item',
    removeChecklistItem: 'Remove item',
    moveChecklistItem: 'Move item',
    extractToCard: 'Extract to New Card',
    sortCards: 'Sort cards',
    sortDueDate: 'Due date',
    sortAlphaAsc: 'Alphabetically (A-Z)',
    sortAlphaDesc: 'Alphabetically (Z-A)',
    sortCustom: 'Custom order',
    archive: 'Archive',
    archivedListsAndCards: 'Archived Lists & Cards',
    returnToBoard: 'Return to Board',
    noArchivedLists: 'No archived lists.',
    noCardsInList: 'No cards in this list.',
    restore: 'Restore',
    deleteConfirmTitle: 'Delete permanently?',
    deleteConfirmMessage: 'Are you sure you want to permanently delete "{{name}}"? This cannot be undone.',
  },
  timeTracker: {
    title: 'Time Tracker',
    start: 'Start',
    end: 'End',
    addEntry: 'Add entry',
    startTitle: 'Start Tracking',
    addTitle: 'Add Time Entry',
    editTitle: 'Edit Time Entry',
    deleteTitle: 'Delete time entry?',
    deleteConfirmMessage: 'Delete the recorded time for "{{name}}"? This cannot be undone.',
    deleteEntry: 'Delete entry',
    editEntry: 'Edit entry',
    currentTask: 'Currently tracking',
    nothingRunning: 'No task is being tracked right now.',
    alreadyRunning: 'Another task is already being tracked. End it first.',
    pickCard: 'Choose a card first.',
    timesRequired: 'Both a start and an end time are required.',
    endBeforeStart: 'The end time must be after the start time.',
    startTime: 'Start',
    endTime: 'End',
    recopyFromCard: 'Copy the details from a card again',
    snapshotHint: 'These were copied from the card when the entry was recorded, so editing them here changes only this entry.',
    commentHint: 'What this stretch of work was about (optional)',
    year: 'Year',
    month: 'Month',
    allMonths: 'All months',
    export: 'Export CSV',
    startColumn: 'Start',
    endColumn: 'End',
    durationColumn: 'Duration',
    hoursColumn: 'Hours',
    boardColumn: 'Board',
    cardColumn: 'Card',
    labelsColumn: 'Labels',
    commentColumn: 'Comment',
    actionsColumn: 'Actions',
    overlapWarning: 'This entry overlaps another one.',
    noEntries: 'No time recorded for this period.',
    total: '{{entries}} entries · {{duration}} total',
  },
  common: {
    or: 'or',
    continueWithGoogle: 'Continue with Google',
    cancel: 'Cancel',
    close: 'Close',
    save: 'Save',
    saveError: 'Something went wrong. Please try again.',
    add: 'Add',
    delete: 'Delete',
  },
} as const

export default en
